...
const ProjectPage = () => {
    ...
    const projectDetails = [
        ...
        },
        {
            ...
            description: [
                ...
                <p key='11'>After spending a significant amount of time and energy (and having a great time along the way), I’m proud to say that my agents won the competition with an overall win rate of ~89%.</p>
            ],
            galleryPaths: [
                {
                    id: uuidv4(),
        ...
        },
        {
            title: 'Rate My Surf',
            description: [
                <p key='1'>The vision of Rate My Surf is to provide surfers in the San Francisco Bay Area with a resource to discover and review beaches. Additionally, current weather conditions are displayed to provide surfers with insight into a surfing location before they visit.</p>,
    ...
    let displayGalleryPaths = [];
    const findContent = () => {
        const projectName = pathname.replace("/Project/", "");
        const project = projectDetails.find(element => element.title == projectName);
        if (project) {
            displayTitle = project.title;
            displayDescription = project.description;
            displayGalleryPaths = project.galleryPaths;
        }
    }
    ...
    return (
        <section>
            ...
            <h2 className='' id="project-title">{displayTitle}</h2>
            {displayGalleryPaths.length > 0 &&
                <ImageCarousel galleryPaths={displayGalleryPaths}></ImageCarousel>
            }
